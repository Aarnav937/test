...
# --- ADDITION --- Loading the new Weather API Key
...
# --- MODIFICATION --- This function now has a print statement for debugging.
def GetWeather(city: str):
    if not WEATHER_API_KEY:
        return "Weather API key not found. Please check the .env file."
    ...
    try:
        ...
        # This new line will print the full server response to your console
        print(f"OpenWeatherMap API Response: {response}")
        ...
        return f"The current temperature in {city} is {temp}° Celsius, but it feels like {feels_like}° Celsius. The sky has {weather_desc}."
...
def YouTubeSearch(topic):
    url = f"https://www.youtube.com/results?search_query=topic{topic.replace(' ', '+')}"
    webbrowser.open(url)
    return True
...
async def TranslateAndExecute(commands: list[str]):
    ...
    for command in commands:
        ...
        # --- ADDITION --- This adds the new weather skill to the execution logic.
...
async def Automation(commands: list[str]):
    print(f"Starting automation with commands: {commands}")
    results = []
    # No need for async for here if TranslateAndExecute already collects all results.
    # We will simply await the collection of all results.
    # However, TranslateAndExecute still yields, so we need to collect them.
    async for result in TranslateAndExecute(commands):
        results.append(result)
    print(f"Automation completed. Results: {results}")
    return results # <--- CHANGE THIS: Return the list of results